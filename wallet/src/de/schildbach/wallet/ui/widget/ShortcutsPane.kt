...
class ShortcutsPane(context: Context, attrs: AttributeSet) : LinearLayout(context, attrs), View.OnClickListener {
    ...
    val joinDashPayButton: ShortcutButton by lazy {
        ShortcutButton(context,
                0,
                R.string.shortcut_join_dashpay,
                this,
    ...
    private val isSmallScreen = resources.displayMetrics.densityDpi <= DisplayMetrics.DENSITY_MEDIUM
    ...
    private fun refresh() {
        ...
        secondaryItems.forEach {
            it.visibility = if (displayed.size < numberOfButtons) {
                if (it == payToContactButton && !showPayToContact) {
                    View.GONE
                } else {
                    displayed.add(it)
                    View.VISIBLE
