...
class PlatformRepo(val walletApplication: WalletApplication) {
    ...
    suspend fun searchContacts(text: String, orderBy: UsernameSortOrderBy): Resource<List<UsernameSearchResult>> {
        return try {
            ...
            log.error("searchContacts: ${e.message}")
            Resource.error(e.localizedMessage ?: e.message ?: "Unknown error", null)
        }
    }
    ...
    suspend fun updateContactRequests(userId: String) {
        ...
        try {
            ...
            walletApplication.configuration.setPrefsLastSeenNotificationTime(walletApplication.configuration.lastSeenNotificationTime - 1000 * 60 * 5)
