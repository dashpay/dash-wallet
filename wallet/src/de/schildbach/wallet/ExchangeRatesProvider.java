...
public class ExchangeRatesProvider extends ContentProvider {
    ...
    private static final URL EXMO_URL;
    private static final String[] EXMO_FIELDS = new String[]{"last_trade"};
    private static final String EXMO_SOURCE = "exmo.com";

    // https://bitmarket.eu/api/ticker

    static {
        try {
            BITCOINAVERAGE_URL = new URL("https://api.bitcoinaverage.com/ticker");
            BLOCKCHAININFO_URL = new URL("https://blockchain.info/ticker");
            EXMO_URL = new URL("https://api.exmo.com/v1/ticker/");
    ...
    public Cursor query(final Uri uri, final String[] projection, final String selection, final String[] selectionArgs, final String sortOrder) {
        ...
        if (!offline && (lastUpdated == 0 || now - lastUpdated > UPDATE_FREQ_MS)) {
            ...
            if (newExchangeRates == null)
                newExchangeRates = requestExchangeRates(EXMO_URL, userAgent, EXMO_SOURCE, EXMO_FIELDS);
            if (newExchangeRates == null)
...
//            btcRate = (Double) result;
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                for (final Iterator<String> i = head.keys(); i.hasNext(); ) {
                    String currencyCode = Strings.emptyToNull(i.next());

                    if (currencyCode != null && currencyCode.toLowerCase().contains("dash") && !
                            currencyCode.toLowerCase().contains("btc") && !"timestamp".equals(currencyCode) && !MonetaryFormat.CODE_BTC.equals(currencyCode)
                            && !MonetaryFormat.CODE_MBTC.equals(currencyCode) && !MonetaryFormat.CODE_UBTC.equals(currencyCode)) {
                        final JSONObject o = head.getJSONObject(currencyCode);
                        currencyCode = currencyCode.replace("DASH_", "");
                        for (final String field : fields) {
                            ...
                            if (rateStr != null) {
                                try {
                                    ...
                                    Log.e("TAG", "requestExchangeRates: " + rateForBTC + " btcRate:" + btcRate);
