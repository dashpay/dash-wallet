...
class CrowdNodeWithdrawalDeniedResponse(networkId: String) : CoinsFromAddressTxFilter(
    CrowdNodeConstants.getCrowdNodeAddress(networkId),
    WITHDRAWAL_DENIED_RESPONSE_CODE
) {
    companion object {
        val WITHDRAWAL_DENIED_RESPONSE_CODE: Dash =
            CrowdNodeConstants.API_OFFSET + Dash.valueOf(ApiCode.WithdrawalDenied.code)
    }
}
