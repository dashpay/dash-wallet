...
class CrowdNodeApiAggregatorTest {
    ...
    private val welcomeResponseTx = TestTx(welcomeData)
    ...
    private val confirmationTx = TestTx(confirmationTxData)
    ...
    private val walletData = mock<WalletDataProvider> {
        ...
        on { networkId } doReturn networkId
    ...
    private val webApi = mock<CrowdNodeWebApi> {
        ...
        onBlocking { resolveBalance(any()) } doReturn Resource.success(Dash.COIN)
    ...
    fun setup() {
        ...
        val welcomeConnectedTx = TestTx(welcomeConnected)
        welcomeResponseTx.connect(0, welcomeConnectedTx)
        ...
        val confirmationConnectedTx = TestTx(confirmationConnected)
        confirmationTx.connect(0, confirmationConnectedTx)
    }
    ...
    fun stopTrackingLinked_doesNotDemoteApiSignUpStatus() {
        runBlocking {
            ...
            val mockFullSet = mock<FullCrowdNodeSignUpTxSet> {
                on { welcomeToApiResponse } doReturn CrowdNodeWelcomeToApiResponse(networkId).apply {
                    matches(welcomeResponseTx.toTxInfo())
                }
            }
            blockchainApi.stub {
    ...
    fun `linked online account restored correctly from confirmation transaction`(): Unit = runBlocking {
        ...
        blockchainApi.stub {
            on { getApiAddressConfirmationTx() } doReturn confirmationTx.toTxInfo()
        }
        webApi.stub {
            onBlocking {
                isApiAddressInUse(any())
            } doReturn Pair(true, "yd9CUc7wvATUS3GfdmcAhRZhG7719jhNf9")
        }
