...
class CoinBaseAuthRepository @Inject constructor(
    ...
    suspend fun getUserToken(
        ...
        val apiResponse = authApi.getToken(code = code).also {
            it.body()?.let { tokenResponse ->
                saveAccessTokens(tokenResponse.accessToken, tokenResponse.refreshToken)
    ...
    suspend fun getUserAccount() = safeApiCall {
        servicesApi.getUserAccount().body()?.let { coinBaseUserAccountInfo ->
            val userAccountData = coinBaseUserAccountInfo.data?.firstOrNull { coinBaseUserAccountData ->
                coinBaseUserAccountData.balance?.currency?.equals(DASH_CURRENCY) ?: false
            }
            userAccountData?.let { accountData ->
                userPreferences.setLastCoinBaseBalance(accountData.balance?.amount)
            }
        }
    }
