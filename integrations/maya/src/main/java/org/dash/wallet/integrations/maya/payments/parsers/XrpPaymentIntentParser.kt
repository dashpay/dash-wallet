...
/**
 * Parser for XRP (Ripple) payment intents. Classic XRP addresses are
 * Base58Check encoded with a leading `r` and are 25-35 characters long.
 */
open class XrpPaymentIntentParser(
    currency: String = "XRP",
    asset: String = "XRP.XRP",
    shortAsset: String? = null
) : MayaPaymentIntentParser(currency, "xrp", asset, shortAsset, null) {
    private val log = LoggerFactory.getLogger(XrpPaymentIntentParser::class.java)
    private val addressParser = XrpAddressParser()
    ...
    override suspend fun parse(input: String): PaymentIntent = withContext(Dispatchers.Default) {
        ...
        log.info("cannot classify as XRP address: '{}'", input)
        throw PaymentIntentParserException(
