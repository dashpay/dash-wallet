...
/**
 * Parser for Solana (SOL) payment intents. Solana addresses are Base58-encoded
 * 32-byte ed25519 public keys (typically 32-44 characters). We only validate
 * the lexical pattern; SwapKit verifies the address at quote time.
 */
open class SolanaPaymentIntentParser(
    currency: String = "SOL",
    asset: String = "SOL.SOL",
    shortAsset: String? = null
) : MayaPaymentIntentParser(currency, "solana", asset, shortAsset, null) {
    private val log = LoggerFactory.getLogger(SolanaPaymentIntentParser::class.java)
    private val addressParser = SolanaAddressParser()
    ...
    override suspend fun parse(input: String): PaymentIntent = withContext(Dispatchers.Default) {
        ...
        log.info("cannot classify as SOL address: '{}'", input)
        throw PaymentIntentParserException(
