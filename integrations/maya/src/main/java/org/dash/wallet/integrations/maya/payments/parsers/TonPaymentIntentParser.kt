...
/**
 * Parser for TON (Toncoin) payment intents. User-friendly TON addresses are
 * Base64URL-encoded and 48 characters long.
 */
open class TonPaymentIntentParser(
    currency: String = "TON",
    asset: String = "TON.TON",
    shortAsset: String? = null
) : MayaPaymentIntentParser(currency, "ton", asset, shortAsset, null) {
    private val log = LoggerFactory.getLogger(TonPaymentIntentParser::class.java)
    private val addressParser = TonAddressParser()
    ...
    override suspend fun parse(input: String): PaymentIntent = withContext(Dispatchers.Default) {
        ...
        log.info("cannot classify as TON address: '{}'", input)
        throw PaymentIntentParserException(
