...
/** Parser for Starknet (STRK) payment intents. Starknet addresses are 0x-prefixed felts up to 64 hex chars. */
open class StarknetPaymentIntentParser(
    currency: String = "STRK",
    asset: String = "STRK.STRK",
    shortAsset: String? = null
) : MayaPaymentIntentParser(currency, "starknet", asset, shortAsset, null) {
    private val log = LoggerFactory.getLogger(StarknetPaymentIntentParser::class.java)
    private val addressParser = StarknetAddressParser()
    ...
    override suspend fun parse(input: String): PaymentIntent = withContext(Dispatchers.Default) {
        ...
        log.info("cannot classify as STRK address: '{}'", input)
        throw PaymentIntentParserException(
