...
/**
 * Parser for TRON (TRX) payment intents. Tron base58check addresses begin
 * with `T` and are 34 characters long.
 */
open class TronPaymentIntentParser(
    currency: String = "TRX",
    asset: String = "TRON.TRX",
    shortAsset: String? = null
) : MayaPaymentIntentParser(currency, "tron", asset, shortAsset, null) {
    private val log = LoggerFactory.getLogger(TronPaymentIntentParser::class.java)
    private val addressParser = TronAddressParser()
    ...
    override suspend fun parse(input: String): PaymentIntent = withContext(Dispatchers.Default) {
        ...
        log.info("cannot classify as TRON address: '{}'", input)
        throw PaymentIntentParserException(
