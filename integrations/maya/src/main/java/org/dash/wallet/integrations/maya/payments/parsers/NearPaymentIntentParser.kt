...
/**
 * Parser for NEAR Protocol payment intents. NEAR accounts can be either
 * implicit (64 lowercase hex characters) or named (e.g. `alice.near`).
 */
open class NearPaymentIntentParser(
    currency: String = "NEAR",
    asset: String = "NEAR.NEAR",
    shortAsset: String? = null
) : MayaPaymentIntentParser(currency, "near", asset, shortAsset, null) {
    private val log = LoggerFactory.getLogger(NearPaymentIntentParser::class.java)
    private val addressParser = NearAddressParser()
    ...
    override suspend fun parse(input: String): PaymentIntent = withContext(Dispatchers.Default) {
        ...
        log.info("cannot classify as NEAR address: '{}'", input)
        throw PaymentIntentParserException(
